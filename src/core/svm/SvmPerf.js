...
SvmPerf.prototype = {
  ...
  getFeatures: function getFeatures() {
    ...
    _.each(this.mapFeatureToWeight, function (weight, index, list) {
      if (parseInt(index) == 0) featlist.push(['bias', weight]);else featlist.push([this.featureLookupTable.numberToFeature(parseInt(index) - 1), weight]);
    }, this);

    featlist = _.sortBy(featlist, function (num) {
      return num[1];
    });
    return featlist;
  },
...
var SVM_PERF_MODEL_PATTERN = new RegExp("[\\S\\s]*" + "^([\\S\\s]*) # threshold b[\\S\\s]*" + // parse the threshold line
...
function modelStringToModelMap(modelString) {
  ...
  var mapFeatureToWeight = {}; //mapFeatureToWeight.threshold = threshold; // not needed - we use our own bias
  ...
  for (var i = 1; i < featuresAndWeights.length; ++i) {
    ...
    if (featureWeight.length != 2) throw new Error("Model featureAndWeight doesn't match svm-perf pattern: featureAndWeight=" + featureAndWeight);
    var feature = parseInt(featureWeight[0]);
    if (feature <= 0) throw new IllegalArgumentException("Non-positive feature id: featureAndWeight=" + featureAndWeight);
    var weight = parseFloat(featureWeight[1]);
    if (Math.abs(weight) >= MIN_WEIGHT) mapFeatureToWeight[feature - FIRST_FEATURE_NUMBER] = weight; // start feature values from 0.
