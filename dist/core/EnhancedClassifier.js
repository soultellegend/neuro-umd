...
/*#__PURE__*/
function () {
  function EnhancedClassifier(opts) {
    _classCallCheck(this, EnhancedClassifier);
  ...
  _createClass(EnhancedClassifier, [{
    ...
    key: "setNormalizer",
    value: function setNormalizer(normalizer) {
      if (normalizer) this.normalizers = Array.isArray(normalizer) ? normalizer : [normalizer];
    }
    /** Set an additional feature extractor, for classification only. */

  }, {
    key: "setFeatureExtractorForClassification",
    value: function setFeatureExtractorForClassification(featureExtractorForClassification) {
    ...
    key: "setFeatureLookupTable",
    value: function setFeatureLookupTable(featureLookupTable) {
    ...
    key: "setLabelLookupTable",
    value: function setLabelLookupTable(labelLookupTable) {
      if (labelLookupTable) {
        this.labelLookupTable = labelLookupTable;
        if (this.classifier.setLabelLookupTable) this.classifier.setLabelLookupTable(labelLookupTable); // for generating clearer explanations only
      }
    ...
    key: "sampleToFeatures",
    value: function sampleToFeatures(sample, featureExtractor) {
      var features = sample;
    ...
    key: "instanceFilter",
    value: function instanceFilter(data) {
    ...
    key: "correctFeatureSpelling",
    value: function correctFeatureSpelling(sample) {
      if (this.spellChecker) {
    ...
    key: "featuresToArray",
    value: function featuresToArray(features) {
      var array = features;
    ...
    key: "countFeatures",
    value: function countFeatures(features) {
    ...
    key: "editFeatureValues",
    value: function editFeatureValues(features, remove_unknown_features) {
      if (this.multiplyFeaturesByIDF) {
        for (var feature in features) {
          var IDF = this.tfidf.idf(feature);
          if (IDF != Infinity) features[feature] *= IDF;else delete features[feature];
        }
    ...
    key: "trainOnline",
    value: function trainOnline(sample, classes) {
      ...
      if (this.pastTrainingSamples) this.pastTrainingSamples.push({
        input: sample,
        output: classes
      });
    }
    ...
    key: "trainBatch",
    value: function trainBatch(dataset) {
      ...
      dataset = dataset.map(function (datum) {
        ...
        if (this.tfidf) this.tfidf.addDocument(features); // this.trainSpellChecker(features);
      ...
      dataset.forEach(function (datum) {
        ...
        /*remove_unknown_features=*/
        false);
        if (featureLookupTable) datum.input = featureLookupTable.hashToArray(datum.input);
      }, this);
    ...
    key: "classifyPart",
    value: function classifyPart(sample, explain, continuous_output) {
      ...
      true);
      var array = this.featuresToArray(features);
      var classification = this.classifier.classify(array, explain, continuous_output); // if (this.spellChecker && classification.explanation) {
    ...
    value: function classify(sample, explain, continuous_output, original, classifier_compare) {
      ...
      if (!this.inputSplitter) {
        ...
        var parts = this.inputSplitter(sample); // var accumulatedClasses = {};
        ...
        parts.forEach(function (part) {
          ...
          var classes = explain > 0 ? classesWithExplanation.classes : classesWithExplanation; // for (var i in classes)
      ...
      if (explain > 0) return {
        classes: classes,
        scores: scores,
        explanation: explanations // bonus: bonus
    ...
    value: function retrain() {
      if (!this.pastTrainingSamples) throw new Error("No pastTrainingSamples array - can't retrain");
    ...
    key: "backClassify",
    value: function backClassify(theClass) {
      if (!this.pastTrainingSamples) throw new Error("No pastTrainingSamples array - can't backClassify");
    ...
    key: "toJSON",
    value: function toJSON(callback) {
      return {
        ...
        spellChecker: this.spellChecker ? this.spellChecker
        /*.toJSON()*/
        : undefined,
...
var normalizeClasses = function normalizeClasses(classes, labelLookupTable) {
  ...
  if (labelLookupTable) classes = classes.map(labelLookupTable.featureToNumber, labelLookupTable);
  classes.sort();
  return classes;
};